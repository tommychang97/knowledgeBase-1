window.onload = function () {
    let i = 0;
    for (
        length = document.getElementsByClassName('showreplies').length;
        i < length;
        i++
    ) {
        document.getElementsByClassName('showreplies')[
            i
        ].onclick = function () {
            ...
            const threadId = divReplyBox.getAttribute('data-threadid');
            console.log(
                'divreplybox threadid',
                divReplyBox.getAttribute('data-threadid')
            );
            ...
            if (numReplies < 1) {
                ...
                let replytext = document.createTextNode(
                    '*Insert exact problem that you have.* EDIT: Nevermind, I solved it! Closed.'
                );
                ...
                form.setAttribute('action', `home/threads/${threadId}/post`);
                const replyBox = document.createElement('textarea');
                replyBox.setAttribute('form', 'replyform');
                replyBox.setAttribute('name', "message");
